...
import { incrementSwipeCount, incrementConnectionCount } from "./activity.controller.js";
...
const sendConnectionRequest = AsyncHandler(async (req, res, next) => {
    ...
    await incrementSwipeCount(senderId, "given");
    await incrementSwipeCount(receiverId, "received");
...
const reviewConnectionRequest = AsyncHandler(async (req, res, next) => {
    ...
    // If accepted, increment connection count for both users
    if (status === "accepted") {
        await incrementConnectionCount(connectionRequestExists.senderId);
        await incrementConnectionCount(receiverId);
    }
    // Return the response
    res.status(200).json({
        success: true,
